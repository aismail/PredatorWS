...
public class OpenCV {
	...
	static {
		try { System.loadLibrary("OpenCV"); }
			...
			if ( OS.indexOf("mac")!=-1 )
				err += "the opencv.framework exists in '/Library/Frameworks' folder";
			if ( OS.indexOf("windows")!=-1 )
				err += "the '\\path\\to\\OpenCV\\bin' exists in your system PATH";
			if ( OS.indexOf("linux")!=-1 )
				err += "'libcxcore.so', 'libcv.so', 'libcvaux.so', 'libml.so', "+
